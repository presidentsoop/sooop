...
export default function DataTable<T extends Record<string, any>>({
    ...
    return (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    {searchable && (
                        <div className="relative flex-1 max-w-full sm:max-w-md">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-2 text-sm bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-sans"
                            />
                        </div>
                    )}
            ...
            {/* Desktop View (Table) */}
            <div className={`overflow-x-auto ${mobileRenderer ? 'hidden md:block' : 'block'}`}>
            ...
            {totalPages > 1 && !disablePagination && (
                <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 bg-gray-50/50">
                    <div className="text-sm text-gray-600">
                        Showing <span className="font-medium hidden sm:inline">{startIndex + 1} - {Math.min(startIndex + pageSize, sortedData.length)}</span>
                        <span className="font-medium sm:hidden">{startIndex + 1}-{Math.min(startIndex + pageSize, sortedData.length)}</span>
                        <span className="hidden sm:inline"> of <span className="font-medium">{sortedData.length}</span></span>
                    </div>
                    <div className="flex items-center gap-1">
                        <button
